...
server <- function(input, output, session) {
  ...
  holidays_upload <- reactive({
    ...
    else h <- read_csv(input$holidays_file$datapath) 
    return(h)
  })
  ...
  forecast <- reactive({
    ...
    predict(prophet_model(),p_future()) %>%
      mutate(ds2 = as_date(ds)) %>% # prophet changed the format (date to POSIXct)
      left_join(datx, by = c("ds2" = "ds")) %>%
  ...
  output$ts_plot <- renderPlot({
    ...
    g+theme_classic() +
      geom_point(aes(y = anom_value), colour = "red", size = 0.1) +
      geom_point(aes(size = log(anom_magnitude+1)), colour = "red", alpha = 0.2) + 
      ylim(0, NA) + 
